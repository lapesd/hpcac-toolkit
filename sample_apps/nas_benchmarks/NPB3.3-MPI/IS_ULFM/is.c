...
// Function that finds the seed value for a random number generator
double find_my_seed(int kn,       // Processor rank, 0<=kn<=num procs
                    int np,       // Number of processors (num procs)
                    long nn,      // Total number of random numbers to be generated across all processors
                    double s,     // Initial seed for RNG, for example, 314159265.00
                    double a)     // RNG multiplier, for example, 1220703125.00
{
    long i;
    ...
    // Calculate the number of random numbers to be generated per processor
    nq = nn / np;

    // Find the largest power of 2 less than or equal to nq
    for (mq = 0; nq > 1; mq++, nq /= 2);

    // Calculate an = a^(2^mq) mod 2^46
    t1 = a;
    for (i = 1; i <= mq; i++)
        t2 = randlc(&t1, &t1);
    an = t1;

    // Initialize variables for seed calculation
    kk = kn;
    t1 = s;
    t2 = an;

    // Calculate the seed based on the processor rank (kn)
    for (i = 1; i <= 100; i++)
    {
        ik = kk / 2;
        // If kk is odd, update t1 using the current value of t2
        if (2 * ik != kk)
            t3 = randlc(&t1, &t2);
        // If kk is zero, break the loop
        if (ik == 0)
            break;
        // Update t2 using its current value
        t3 = randlc(&t2, &t2);
        kk = ik;
    }
...
/*************      C  R  E  A  T  E  _  S  E  Q      ************/
/*****************************************************************/
// Function that creates a sequence of random numbers using the provided seed and multiplier
void create_seq(double seed, double a)
{
    double x; // Variable to store the generated random number
    int i, k; // Loop variables

    // Set k to a quarter of the maximum key value
    k = MAX_KEY / 4;

    // Loop through the number of keys to generate
    for (i = 0; i < num_keys; i++)
    {
        // Generate random numbers using the seed and multiplier, and sum them up
        x = randlc(&seed, &a);
        x += randlc(&seed, &a);
        x += randlc(&seed, &a);
        x += randlc(&seed, &a);

        // Store the generated random number in the key_array, scaled by k
        key_array[i] = k * x;
    }
}
...
void full_verify(void)
{
    ...
    // Now, finally, sort the keys locally:
    ...
    // Send largest key value to the next processor
    if (my_rank > 0)
        MPI_Irecv(&k,
                  1,
                  MP_KEY_TYPE,
    ...
    // Report the number of keys out of sort, if any
    if (j != 0)
    {
